...
NETLIST_START(main)
{
	...
	// ---- Phase 5 fault scenario ----
	// Force FB_ADDR_CTR_QB into STUCK_LO. Effect: address bit B is held
	// low, so the decoder only ever sees B=0 — ROM_CSn and RAM_CSn cycle
	// at twice their healthy rate, while POKEY_CSn and EAROM_CSn never
	// assert. Real-cabinet symptom: POKEY (sound chip) and EAROM
	// (high-score memory) appear dead even though the rest of the bus
	// works fine.
	PARAM(FB_ADDR_CTR_QB.MODE, 2)   // 0=NORMAL 1=STUCK_HI 2=STUCK_LO 3=OPEN
}

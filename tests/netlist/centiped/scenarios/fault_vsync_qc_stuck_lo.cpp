...
NETLIST_START(main)
{
	...
	NET_C(FB_CLK_Q.Y,    H_HI.CLK)
	FAULT_BUFFER(FB_H_LO_RC, H_LO.RC)
	NET_C(FB_H_LO_RC.Y, H_HI.ENP)
	NET_C(FB_H_LO_RC.Y,  H_HI.ENT)
	NET_C(HI,       H_HI.CLRQ)
	NET_C(HI,       H_HI.LOADQ)
	NET_C(LO,       H_HI.A)
	NET_C(LO,       H_HI.B)
	...
	NET_C(FB_CLK_Q.Y,    V_LO.CLK)
	NET_C(FB_H_LO_RC.Y,  V_LO.ENP)
	FAULT_BUFFER(FB_H_HI_RC, H_HI.RC)
	NET_C(FB_H_HI_RC.Y, V_LO.ENT)
	NET_C(HI,       V_LO.CLRQ)
	NET_C(HI,       V_LO.LOADQ)
	NET_C(LO,       V_LO.A)
	NET_C(LO,       V_LO.B)
	...
	FAULT_BUFFER(FB_H_HI_QD, H_HI.QD)
	NET_C(FB_H_HI_QD.Y, HSYNC_DEC.A)
	FAULT_BUFFER(FB_H_HI_QC, H_HI.QC)
	NET_C(FB_H_HI_QC.Y, HSYNC_DEC.B)
	FAULT_BUFFER(FB_H_HI_QB, H_HI.QB)
	NET_C(FB_H_HI_QB.Y, HSYNC_DEC.C)
	ALIAS(HSYNC_n, HSYNC_DEC.Q)

	// ---- VSYNC_n decode: low when V >= 12 (V[3:2] = 11) ----
	FAULT_BUFFER(FB_V_LO_QD, V_LO.QD)
	NET_C(FB_V_LO_QD.Y, VSYNC_DEC.A)
	FAULT_BUFFER(FB_V_LO_QC, V_LO.QC)
	NET_C(FB_V_LO_QC.Y, VSYNC_DEC.B)
	...
	// Force FB_V_LO_QC into STUCK_LO. Effect: V[2] is held low, so the
	// VSYNC NAND inputs become (V[3], 0) which evaluates to logic-high
	// always. VSYNC_n therefore never asserts (no vertical sync) — the
	// real-cabinet symptom is a freely rolling picture.
	PARAM(FB_V_LO_QC.MODE, 2)   // 0=NORMAL 1=STUCK_HI 2=STUCK_LO 3=OPEN
}
